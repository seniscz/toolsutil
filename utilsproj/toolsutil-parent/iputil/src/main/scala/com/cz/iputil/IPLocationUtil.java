...
public class IPLocationUtil {
    ...
    /**
     * @param path 纯真IP数据库保存的路径
     * @Description 有参构造
     */
    public IPLocationUtil(String path) {
        IPFile = path;
        INSTALL_DIR = path;
    }
    ...
    public void init() {
        try {
            ...
            try {
                ...
                // 如果找不到这个文件，再尝试再当前目录下搜索，这次全部改用小写文件名 ,因为有些系统可能区分大小写导致找不到ip地址信息文件
                String filename = new File(IPFile).getName().toLowerCase();
     ...
     * @param ip
     * @return
     * @Description 查询IP地址位置 - synchronized的作用是避免多线程时获取区域信息为空
     */
     ...
     * @Description 从offset位置读取4个字节为一个long，因为java为big-endian格式，所以用此函数来做转换
     */
     ...
     * @return 读取的long值，返回-1表示读取文件失败
     * @Description 从offset位置读取3个字节为一个long，因为java为big-endian格式，所以用此函数来做转换
     */
     ...
     * @param offset
     * @param ip
     * @Description 从offset位置读取四个字节的ip地址放入ip数组中，读取后的ip为big-endian格式，但是文件中是little-endian形式，将会进行转换
     */
     ...
     * @param offset 国家记录的起始偏移
     * @return IPLocation对象
     * @Description 给定一个ip国家地区记录的偏移，返回一个IPLocation结构，此方法应用与内存映射文件方式
     */
    ...
    public static void main(String[] args) {
        IPLocationUtil ip = new IPLocationUtil("F:\\qqwry.dat");
        ip.init();
        String address = "1.183.255.255"; // 192.168.42.189 58.19.25.0  112.225.35.70   1.83.146.198  1.183.255.255  056.000.000.000      49.246.255.255

        System.out.println("获取到的区域信息:  " + ip.getIPLocation(address).getCountry());
        System.out.println("获取到的省:  " + ip.getIPLocation(address).getProvince());
        System.out.println("获取到的城市:   " + ip.getIPLocation(address).getCity());
        System.out.println("getArea 的值为： " + ip.getIPLocation(address).getArea());
        System.out.println("getAddress 的值为： " + ip.getIPLocation(address).getAddress());
    }
